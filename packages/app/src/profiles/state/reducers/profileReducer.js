...
export function currentProfileReducer(state = CurrentProfile, action) {
  switch (action.type) {
    ...
    case PROFILE_ACTIONS.TRANSFER: {
      if (JSON.stringify(state.billing) === JSON.stringify(state.shipping)) {
        return state;
      }
...
export function selectedProfileReducer(state = SelectedProfile, action) {
  switch (action.type) {
